...
BEGIN
	...
	-- 金币操作 ------------------------------------------------------------------------------------
	-- 锁定解除
	DELETE ZQTreasureDBLink.ZQTreasureDB.dbo.GameScoreLocker WHERE UserID=@dwUserID

	-- 累加金币
	UPDATE ZQTreasureDBLink.ZQTreasureDB.dbo.GameScoreInfo SET Score=Score+@lGameGold,InsureScore=InsureScore+@lInsureScore WHERE UserID=@dwUserID

	IF @@ROWCOUNT=0
	BEGIN
		-- 赠送金币
		INSERT ZQTreasureDBLink.ZQTreasureDB.dbo.GameScoreInfo(UserID, RegisterIP,LastLogonIP)
	...
	-- 用户魅力
	INSERT ZQTreasureDBLink.ZQTreasureDB.dbo.RecordUserLoveliness (UserID, KindID, ServerID,Loveliness,ClientIP)
	VALUES (@dwUserID,@wKindID,@wServerID,@lLoveliness,@strClientIP)

	-- 金币操作 ------------------------------------------------------------------------------------

	-- 用户经验
	UPDATE ZQGameUserDBLink.ZQGameUserDB.dbo.AccountsInfo SET Experience=Experience+@lExperience,Loveliness=@lLoveliness WHERE UserID=@dwUserID
