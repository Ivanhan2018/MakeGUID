...
BEGIN
	SET @TradeType=1
	SELECT @InsureScore=InsureScore FROM ZQTreasureDBLink.ZQTreasureDB.dbo.GameScoreInfo WHERE UserID=@dwUserID

--	UPDATE ZQTreasureDBLink.ZQTreasureDB.dbo.GameScoreInfo SET Score=Score-@lSwapScore,InsureScore=InsureScore+@lSwapScore
--	WHERE UserID=@dwUserID

	SELECT @SourceGameID=GameID,@SourceAccounts=Accounts FROM ZQGameUserDBLink.ZQGameUserDB.dbo.AccountsInfo
	WHERE UserID=@dwUserID

	INSERT ZQTreasureDBLink.ZQTreasureDB.dbo.RecordInsure(KindID,ServerID,SourceUserID,SourceGameID,SourceAccounts,InsureScore,SwapScore,TradeType,ClientIP)
