...
BEGIN
	...
	BEGIN
		...
		FROM WHGameUserDB.dbo.MemberInfo WHERE UserID=@dwRcvUserID
		
		-- 首次购买
		IF @MemberCount IS NULL OR @MemberCount<=0
		BEGIN
			INSERT WHGameUserDB.dbo.MemberInfo (UserID,MemberOrder,MemberOverDate)
		...
		BEGIN	
			...
			FROM WHGameUserDB.dbo.MemberInfo WHERE UserID=@dwRcvUserID AND MemberOrder>=@CurMemberOrder		
			...
			-- 更新同级别会员
			UPDATE WHGameUserDB.dbo.MemberInfo SET MemberOverDate=DATEADD(ss, @dwOnceCount*@dwPachurseCount, @UpMemberOverDate)
			...
			-- 更新下级会员
			UPDATE WHGameUserDB.dbo.MemberInfo SET MemberOverDate=DATEADD(ss, @dwOnceCount*@dwPachurseCount, MemberOverDate)
			WHERE UserID=@dwRcvUserID AND MemberOrder<@CurMemberOrder
		END		
		--------------------------------------------------------------
		...
		-- 附加会员信息
		UPDATE WHGameUserDB.dbo.AccountsInfo
...
BEGIN
	...
	BEGIN		
		...
		FROM WHGameUserDB.dbo.MemberInfo WHERE UserID=@dwUserID	
		...
		BEGIN
			...
			FROM WHGameUserDB.dbo.MemberInfo
			...
			FROM WHGameUserDB.dbo.MemberInfo WHERE UserID=@dwUserID AND MemberOrder<@MaxMemberOrder
