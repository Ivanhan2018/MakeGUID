...
void CDataManageDlg::DoDataExchange(CDataExchange* pDX)
{
	...
	DDX_Control(pDX, IDC_EDIT4, m_edSrc);
	DDX_Control(pDX, IDC_EDIT5, m_edEncrypt);
}
...
void CDataManageDlg::OnBnClickedButton1()
{
	...
	CString strSrc;
	m_edSrc.GetWindowText(strSrc);
	if(strSrc.IsEmpty())
	{
		m_csInfo.SetWindowText(L"加密数据不能为空，请重新输入！");
		m_edSrc.SetFocus();
		return;
	}

	string str=PUBLIC::ws2s(strSrc.GetString());
	CHAR sz[512]={0};
	CXOREncryptA::EncryptData(str.c_str(),sz,sizeof(sz));
	wstring wstr=PUBLIC::s2ws(sz);
	m_edEncrypt.SetWindowText(wstr.c_str());
...
void CDataManageDlg::OnBnClickedButton2()
{
	...
	if(strSrc.IsEmpty())
	{
		m_csInfo.SetWindowText(L"解密数据不能为空，请重新输入！");
		m_edSrc.SetFocus();
		return;
	}
	...
	string str=PUBLIC::ws2s(strSrc.GetString());
	CHAR sz[512]={0};
	CXOREncryptA::CrevasseData(str.c_str(),sz,sizeof(sz));
	wstring wstr=PUBLIC::s2ws(sz);
	m_edEncrypt.SetWindowText(wstr.c_str());
...
void CDataManageDlg::OnBnClickedButton3()
{
	...
	WCHAR sz[512]={0};
	CXOREncryptW::EncryptData(strSrc.GetString(),sz,sizeof(sz));
	m_edEncrypt.SetWindowText(sz);
	return;
...
void CDataManageDlg::OnBnClickedButton4()
{
	...
	WCHAR sz[512]={0};
	CXOREncryptW::CrevasseData(strSrc.GetString(),sz,sizeof(sz));
	m_edEncrypt.SetWindowText(sz);
	return;
	CDataManageDlg *pDataManageDlg=((CDataManageDlg *)theApp.m_pMainWnd);
	//if(!pDataManageDlg||!pDataManageDlg->m_pClientComm)
