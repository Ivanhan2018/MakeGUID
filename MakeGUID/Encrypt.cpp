...
//生成密文
void CMD5Encrypt::EncryptData(LPCSTR pszSrcData, CHAR szMD5Result[16])
{
	//加密密文
	CMD5 MD5Encrypt;
	UCHAR szResult[16];
	MD5Encrypt.MD5Update((unsigned char *)pszSrcData,lstrlenA(pszSrcData)*sizeof(TCHAR));
...
//生成密文
WORD CXOREncryptA::EncryptData(LPCSTR pszSrcData, LPSTR pszEncrypData, WORD wSize)
{
	//效验参数
	ASSERT(wSize>0);
	...
	szRandKey[0]=lstrlenA(pszSrcData);
	...
	for (WORD i=0;i<wTimes;i++)
	{
		...
		_snprintf(pszEncrypData+i*4,5,"%02x%02x",szRandKey[i%ENCRYPT_KEY_LEN],bTempCode);
	}
...
//解开密文
WORD CXOREncryptA::CrevasseData(LPCSTR pszEncrypData, LPSTR pszSrcData, WORD wSize)
{
	//效验参数
	ASSERT(wSize>0);
	ASSERT(pszSrcData!=NULL);
	...
	//效验长度
	WORD wEncrypPassLen=lstrlenA(pszEncrypData);
	...
	for (int i=0;i<wSoureLength;i++)
	{
		...
		szTempBuffer[2]=0;
		UINT uKey=strtol((LPCSTR)szKeyBuffer,&pEnd,16);
...
//生成密文
WORD CXOREncryptW::EncryptData(LPCWSTR pszSourceData, LPWSTR pszEncrypData, WORD wMaxCount/*unsigned char * pszEncrypData, unsigned char* pszSrcData, WORD wSize*/)
{
	//效验参数
//	ASSERT(wSize>0);
//	ASSERT(pszSrcData!=NULL);
//	ASSERT(pszEncrypData!=NULL);
	...
	//步骤准备
	WORD wTempCode=0;
	WORD wTimes=((wRandKey[0]+8-1)/8)*8;
	...
	for (WORD i=0;i<wTimes;i++)
	{
		if (i<wRandKey[0]) wTempCode=strSrcData[i]^wRandKey[i%8];
		else wTempCode=wRandKey[i%8]^(WORD)(rand()%0xFFFF);
		swprintf(szEncrypData+i*8,L"%04X%04X",wRandKey[i%8],wTempCode);
	}
...
//解开密文
WORD CXOREncryptW::CrevasseData(LPCWSTR pszEncrypData, LPWSTR pszSourceData, WORD wMaxCount/*unsigned char * pszEncrypData, unsigned char* pszSrcData, WORD wSize*/)
{
	//效验参数
//	ASSERT(wSize>0);
//	ASSERT(pszSrcData!=NULL);
//	ASSERT(pszEncrypData!=NULL);
	...
	WCHAR szSrcData[64]=L"";

	//效验长度
	WORD wEncrypPassLen=lstrlenW(strEncrypData);
	if (wEncrypPassLen<8*8) return false;
	...
	//长度效验
	ASSERT(wEncrypPassLen==(((wSoureLength+8-1)/8)*8*8));
	if (wEncrypPassLen!=(((wSoureLength+8-1)/8)*8*8)) return false;
